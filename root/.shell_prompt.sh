...
function __promptline_ps1 {
  ...
  # section "a" slices
  __promptline_wrapper "\A" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline {
  ...
  local z_fg="${wrap}38;5;189${end_wrap}"
  local z_bg="${wrap}48;5;25${end_wrap}"
  local z_sep_fg="${wrap}38;5;25${end_wrap}"
  if [[ -n ${ZSH_VERSION-} ]]; then
